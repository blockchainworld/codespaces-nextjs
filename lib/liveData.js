...
export async function getLiveNews() {
  const { liveMode, newsProvider } = getLiveProviderStatus()

  if (!liveMode || !newsProvider) {
    return []
  }

  if (newsProvider !== 'newsapi') {
    throw new Error(`Unsupported news provider: ${newsProvider}`)
  }

  const apiKey = process.env.NEWSAPI_KEY

  if (!apiKey) {
    throw new Error('NEWSAPI_KEY is required when PREDICTINFO_NEWS_PROVIDER=newsapi')
  }
