import officialFeeds from '../data/official-feeds.json'
...
function normalizeRssArticle(item, feedConfig) {
  ...
  const sourceTitle = feedConfig?.label || extractXmlTag(item, 'source') || 'RSS feed'
  ...
  return {
    ...normalized,
    desk: feedConfig?.desk || normalized.desk,
    source: sourceTitle,
    sourceQuality: feedConfig?.sourceQuality || normalized.sourceQuality,
    liveMetadata: {
      source: 'rss',
      feedId: feedConfig?.id || null,
      feedKind: feedConfig?.kind || 'rss',
      overlaid: true,
    },
  }
}
...
function getConfiguredRssFeeds() {
  const selectedIds = parseList(
    process.env.PREDICTINFO_RSS_FEED_IDS,
    officialFeeds.filter((feed) => feed.enabledByDefault).map((feed) => feed.id)
  )
  const byId = new Map(officialFeeds.map((feed) => [feed.id, feed]))
  const configuredFromIds = selectedIds.map((id) => byId.get(id)).filter(Boolean)
  const directUrls = parseList(process.env.PREDICTINFO_RSS_FEEDS)
  const configuredDirectFeeds = directUrls.map((url, index) => ({
    id: `custom-rss-${index + 1}`,
    label: url,
    kind: 'custom',
    desk: 'General',
    sourceQuality: 'Configured RSS feed',
    url,
  }))

  return [...configuredFromIds, ...configuredDirectFeeds]
}

async function getRssNews() {
  const feedConfigs = getConfiguredRssFeeds()
  const limit = Number.parseInt(process.env.PREDICTINFO_RSS_LIMIT || '20', 10)

  const settled = await Promise.allSettled(feedConfigs.map((feed) => fetchRssFeed(feed.url)))
  const items = settled.flatMap((result, index) => {
    ...
    const feedConfig = feedConfigs[index]
    const sourceTitle = extractXmlTag(xml, 'title')

    return extractRssItems(xml).map((item) =>
      normalizeRssArticle(item, {
        ...feedConfig,
        label: feedConfig?.label || sourceTitle,
      })
    )
  })
...
export function getLiveProviderStatus() {
  ...
  return {
    ...
    newsProvider,
    newsProviders,
    rssFeedIds: getConfiguredRssFeeds().map((feed) => feed.id),
